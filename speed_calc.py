...
def calcSpeedTune(jamire_bonus_speed, use_lead):
    ...
    # --- homunl ---
    homunl_total = int( jamire_final_speed - 1 - (HOMUNL_BASE_SPD * base_mult) ) - 1 # = jamire final - homun lead+totem bonus
    ...
    # --- Lushen ---
    LUSHEN_BASE_BONUS = LUSHEN_BASE_SPD * (1 + base_mult) # 103 * 1.39
    lushen_rune_bonus = 0 # current rune bonus needed
    
    while (LUSHEN_BASE_BONUS + lushen_rune_bonus) * SPD_BUFF <= jamire_final_speed:
        lushen_rune_bonus += 1 # calculating the minimum speed required from runes
        
    lushen_total = LUSHEN_BASE_SPD + lushen_rune_bonus # readable total (103 + rune bonus)
...
# Here we calc for a Jamire with a bonus of 200 from runes and using it's lead
